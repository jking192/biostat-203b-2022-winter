...
labevents_tble <- read_csv(
  ...
  col_select = c(subject_id, itemid, valuenum),
  lazy = TRUE
)
item <- c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931, 50960, 50893)
labevents_tble <- labevents_tble %>%
  semi_join(icustays_tble) %>%
  filter(itemid %in% item)
```
...
# labevents_dict
# labevents_tble_wide <- labevents_tble %>%
#   left_join(labevents_tble, labevents_dict, by = "itemid")
labevents_tble <- labevents_tble %>%
  mutate (class = case_when(itemid == 50912 ~ "Creatinine",
                           itemid == 50971 ~ "Potassium",
                           itemid == 50983 ~ "Sodium",
                           itemid == 50902 ~ "Chloride",
                           itemid == 50882 ~ "Bicarbonate",
                           itemid == 51221 ~ "Hematocrit",
                           itemid == 51301 ~ "White Blood Cell Count",
                           itemid == 50931 ~ "Glucose",
                           itemid == 50960 ~ "Magnesium",
                           itemid == 50893 ~"Calcium"))
labevents_tble %>% 
  group_by(class) %>%
  summarize (
    n = n(),
...
item <- c(220045, 220181, 220179, 223761, 220210)
...
chartevents_tble_wide <- chartevents_tble %>%

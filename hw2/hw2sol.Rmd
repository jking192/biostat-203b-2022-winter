...
# Make boxplots for each Lab Measurment unfiltered
...
# Make boxplots for each Lab Measurement filtered using 1.5 IQR rule
for (i in 2:11)
{
  # Temporary variables needed to make plots
  name <- colnames(labevents_tble[,i])
...
rm(name, col, col_vec, iqr, med, sd, q1, q3)
```
...
# Make boxplots for each Vital Measurement filtered using 1.5 IQR rule
for (i in 2:6)
{
  # Temporary variables needed to make plots
  name <- colnames(chartevents_tble[,i])
...
```{r}
# Age at admission
age = year(mimic_icu_cohort$admittime) - 
  (mimic_icu_cohort$anchor_year - mimic_icu_cohort$anchor_age)
#Ethnicity Chart
mimic_icu_cohort %>%
  ggplot() +
  geom_bar(mapping = aes(x = thirty_day_mort, fill = thirty_day_mort)) +
  theme(legend.position = "none") +
  labs(title = "30 Day Mortality by Ethnicity", x = "Ethnicity") +
  theme(plot.title = element_text(hjust = 0.5)) +
  facet_wrap(~ ethnicity, scales = "free", nrow = 3)
# Make language chart
mimic_icu_cohort %>%
  ggplot() +
  geom_bar(mapping = aes(x = thirty_day_mort, fill = thirty_day_mort)) +
  theme(legend.position = "none") +
  labs(title = "30 Day Mortality by Language", x = "Language") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
mimic_icu_cohort %>%
  ...
  geom_bar(mapping = aes(x = thirty_day_mort, fill = thirty_day_mort)) +
  theme(legend.position = "none") +
  labs(title = "30 Day Mortality by Insurance", x = "Insurance") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
mimic_icu_cohort %>%
  ...
  geom_bar(mapping = aes(x = thirty_day_mort, fill = thirty_day_mort)) +
  theme(legend.position = "none") +
  labs(title = "30 Day Mortality by Marital Status", x = "Marital Status") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
mimic_icu_cohort %>%
  ...
  geom_bar(mapping = aes(x = thirty_day_mort, fill = thirty_day_mort)) +
  theme(legend.position = "none") +
  labs(title = "30 Day Mortality by Gender", x = "Gender") +
  theme(plot.title = element_text(hjust = 0.5)) +
  facet_wrap(~ gender, scales = "free", nrow = 2)

# Make Age of admission chart
mimic_icu_cohort %>%
  ...
  theme(legend.position = "none") +
  labs(title = "Distribution of Age of Admission by 30 Day Mortality", 
    y= "Age (years)",
    x = "30 Day Mortality"
  ) +
  theme(plot.title = element_text(hjust = 0.5)) 
...
group_by(mimic_icu_cohort, thirty_day_mort) %>% summarize (n=n())
```

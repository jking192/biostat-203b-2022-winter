...
Prejudice*. `"$1"` is the first argument passed to the `middle.sh` script, 
`"$2"` is thesecond argument passed to the script, and `"$3"` is the third 
argument. When executing the script above, `pg42671.txt` is the first argument, 
`20` is the second argument, and `5` is the third argument. Therefore,the 
commands being executed above, according to the script is:`head -n 20 
pg42671.txt | tail -n 5`. The first line of the shell script specifies that the 
script should always be run with the Bourne Shell (sh).

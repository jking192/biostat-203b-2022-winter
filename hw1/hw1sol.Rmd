...
- `cat` prints the contents of gzip compressed file.

- `zless` is used to view the contents of a gzip compressed file screen by 
screen.The spacebar can be used to move to the next page, but the mouse can also 
be used to scroll through the file. Press `q` to exit.
...
### Solution
- `cal` displays a calendar of the current month with the current day 
highlighted.

- `cal 2021` prints out a calendar of the year 2021.

- `cal 9 1752` prints out a calendar of September 1752. September 3-13 are not 
shown on the calendar.

- `date` prints out the abbreviated day of the week, abbreviated month, day of 
the month, time seperated by colons in the form hr:min:sec, the abbreviated
time zone name, and the current year.

- `hostname` prints the current hostname. Currently, the hostname is
`biostat-203b-teaching-server`.

- `arch` prints the current system architecture for the server. The teaching 
server runs a 64-bit implementation of the x86 architecture set, making the
architecture x86_64.

- `uname -a` outputs the os kernel name, the hostname, the kernel release,
the last time the kernel was compiled in the format of the `date` command, 
the machine architecture, the processor architecutre, the computer architecture,
and the current operating system.

- `uptime` outputs the current time in the form hr:min:sec, how long the 
system (in this case, the teaching server) has been up in the format days,
hr:min, the number of users currently logged on, and the load averages 
over the past 1, 5, and 15 minutes,  which describe the average number of 
processes that are currently being executed or waiting to be executed.
...
including idle time, JCPU time (time used by all proccess on terminal), PCPU 
...
group ids (if they exist), and security context. On the server, my user ID is 
1017, Since I am only in one group,this command only outputs the primary group 
id (1019).
...
- Note that `cal`, `date`, `uptime`, and `w` depend on time of execution, and
`hostname`, `arch`, `uname -a`, `uptime`, `who`, `w`, `id`, `last | head`, 
`history | tail` depends on linux system, user, or both, so output presented in 
the html file could be very different from the actual output when this Rmd is 
re-knit.

- Both `who am i` and `history | tail` do not work on the teaching server when
knitted, so my answers were based on the output I got on my laptop's terminal.
